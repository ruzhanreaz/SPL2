...
import useDoctorJoinedConsultationListener from "../../hooks/useDoctorJoinedConsultationListener";
    ...
    >
      ...
      <div className="flex items-center gap-3">
        <div className="relative">
          <Avatar name={appointment.doctorName} selected={isSelected} />
          {isActive && (
            <span
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-[11px] text-gray-400 mt-0.5">
            {filteredConversations.length} doctor
        ...
        <button
          type="button"
          onClick={onBook}
          className="inline-flex items-center gap-1.5 rounded-full bg-primary-600 px-3 py-1.5 text-xs font-semibold text-white transition-colors hover:bg-primary-700 shadow-sm shadow-primary-200"
        >
...
const RoomPlaceholder = () => (
  <div className="h-full flex flex-col items-center justify-center text-center px-8 bg-gradient-to-b from-gray-50/80 to-white">
    ...
    <p className="text-sm text-gray-400 max-w-xs leading-relaxed">
      Choose a doctor from the list to open your consultation room.
    </p>
  </div>
);

/* ── Main component ─────────────────────────────────────── */
const PatientTelemedicineHub = () => {
  ...
  const filteredConversations = useMemo(() => {
    ...
    return base.filter((conversation) => {
      const appointment = conversation.latestAppointment;
      const text = `${appointment.doctorName || ""} ${appointment.doctorSpecialization || ""} ${appointment.appointmentTime || ""} ${appointment.notes || ""}`;
  ...
  const handleDoctorJoinedConsultation = useCallback((payload) => {
    window.dispatchEvent(
      new CustomEvent("vb:doctor-joined-consultation", {
        detail: payload,
      }),
    );
  }, []);

  useDoctorJoinedConsultationListener({
    token,
    enabled: Boolean(token),
    appointmentId: selectedConversation?.appointmentForRoom?.appointmentId,
    onDoctorJoined: handleDoctorJoinedConsultation,
  });
    ...
    <DashboardLayout>
      <div className="flex flex-col h-[calc(100dvh-80px)] overflow-hidden">
        {appointments.length === 0 ? (
          <PageEmptyState
            icon={Video}
            title="No online consultations scheduled"
            description="Book an online appointment with a doctor to start your telemedicine consultation"
            action={
              <button
                onClick={() => navigate("/patient/find-doctor")}
                className="bg-primary-600 hover:bg-primary-700 text-white font-semibold px-5 py-2 rounded-full transition-colors text-sm shadow-sm shadow-primary-200"
              >
                Find a Doctor
              </button>
            }
            ...
            <div className="hidden lg:flex h-full">
              ...
              <div className="w-[300px] xl:w-[320px] shrink-0 border-r border-gray-100 h-full overflow-hidden">
                <Sidebar
                  ...
                  onBook={() => navigate("/patient/find-doctor")}
              ...
              {selectedConversation ? (
                <>
                  ...
                  <div className="shrink-0 flex items-center gap-3 px-4 py-3 border-b border-gray-100 bg-white shadow-sm z-10">
                    ...
                    <Avatar
                      name={selectedConversation.latestAppointment.doctorName}
                      size="sm"
                    />
                    <div className="min-w-0 flex-1">
                      <p className="text-sm font-semibold text-gray-900 truncate leading-tight">
                        Dr. {selectedConversation.latestAppointment.doctorName}
                      </p>
...
export default PatientTelemedicineHub;
